...
function idcapt()
{
    ...
    $reponse = $bdd->prepare('SELECT id_capteur,etat FROM capteur WHERE id_capteur IN(SELECT id_piece FROM piece WHERE id_piece IN(SELECT id_habitation FROM habitation WHERE id_maison IN (SELECT id_user FROM habitation WHERE id_user=:id_utilisateur))) AND type =:type_demande');
